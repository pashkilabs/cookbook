 ...
 *
 * ---------------------------------------------------------------------------
 * Seven roles, and what was left out
 * ---------------------------------------------------------------------------
 *
 * The original five could not describe 9% of the corpus, and tuning role accuracy against a list
 * that cannot name the answer would be tuning against the wrong target. Two were added:
 *
 *   `marinade`  applied before cooking and largely not eaten as such. Different from a sauce in a
 *               way that matters here — a marinade moves to another protein far more freely than
 *               a sauce does, which is exactly the judgement blending has to make.
 *   `sweet`     the substance a dessert is made of: a posset, a cookie dough, a batter. NOT a
 *               frosting or a filling, which are applied to a base and are sauces by function.
 *
 * Left out on purpose. `drink` — one recipe in the corpus, and a drink is a whole dish rather
 * than a component, so its single component takes null. `dough` — a dough *is* the carbohydrate,
 * and adding it splits a role that already works. `aromatics` or `spice blend` — spices belong to
 * whatever they season, and separating them would give every recipe an extra component nobody
 * wants to blend. `dairy`, `fat` — ingredient categories, not roles in a dish.
 *
 * Seven is near the limit. **A role list that grows to fit every dish stops being a filter**, and
 * null stays available for the dishes none of them fit rather than forcing a seventh guess.
 */
export interface LabelledComponent {
  ...
  from: number;
  to: number;
  role: "protein" | "carbohydrate" | "sauce" | "vegetable" | "garnish" | "marinade" | "sweet";
}
...
export const COMPONENT_LABELS: LabelledRecipe[] = [
  ...
  { id: "ad141281-0140-43b5-a2a3-4ce76c0d1408", title: "Greek Chicken Bowl", ingredientCount: 16, components: [{ name: "marinade", from: 0, to: 8, role: "marinade" }, { name: "quinoa", from: 9, to: 9, role: "carbohydrate" }, { name: "chicken", from: 10, to: 10, role: "protein" }, { name: "salad", from: 11, to: 15, role: "vegetable" }] },
  ...
  { id: "ecb35a4f-386f-4b6b-8b1e-b4289c264cb0", title: "Peach Posset", ingredientCount: 7, components: [{ name: "posset", from: 0, to: 4, role: "sweet" }, { name: "to serve", from: 5, to: 6, role: "garnish" }] },
  { id: "fc3c3f01-56d7-48e5-a4ab-80870c5451f8", title: "Jalape\u00f1o Lime Marinade", ingredientCount: 7, components: [{ name: "marinade", from: 0, to: 6, role: "marinade" }] },
  ...
  { id: "99c35758-4593-4b1d-b757-57315e792751", title: "Lemon-Pepper Marinade", ingredientCount: 6, components: [{ name: "marinade", from: 0, to: 5, role: "marinade" }] },];
